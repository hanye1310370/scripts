...
do
  ...
  /usr/local/mysql/bin/mysql  -u -p  -e "show full processlist"|cut -f6- |grep -v "NULL" | awk '{if ($1>=4) {print $0}}'|grep -v "Time" |tr "\t" " "|awk '{print "{""\"""RUN_TIME""\""":",$1", ""\"""DATA_TYPE""\""":","\""$2$3"\""", ""\"""SQL_MES""\""":","\""$0"\"""}"}' >>/home/mysql_slow_log_$(date +%F).txt
